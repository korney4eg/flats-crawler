...
con = Mysql2::Client.new(:host => "localhost", :username => "flat", :password => 'flat',:database => 'flats')

def update_price(con,code,address,price,rooms,year)
  rs = con.query("SELECT code,price from global where code=#{code};")
  time = Time.new
  if rs.size == 0
    #if item is not in database, then lets record it
    con.query("INSERT INTO global(code,address,price,rooms,year) VALUES (#{code},\'#{address}\',#{price},\"#{rooms}\",#{year});")
    con.query("INSERT INTO price_history VALUES (#{code},#{price},\"#{time.year}-#{time.month}-#{time.day}\");")
  elsif con.query("SELECT code,price from price_history where code=#{code};").first['price'].to_i != price
    #if item is already in database check if price changed
    log "INSERT INTO price_history VALUES (#{code},#{price},\"#{time.year}-#{time.month}-#{time.day}\");", 4
    con.query("INSERT INTO price_history VALUES (#{code},#{price},\"#{time.year}-#{time.month}-#{time.day}\");")
...
(PRICE[0]..PRICE[1]).step(STEP) do |pr|
  ...
  #  printf( "|%8d|%40s|%9d|%4s|%5d|\n",code,address,price,rooms,year)
  #  puts "@#{price}@"
  end
end
#puts page.css('div#pager-top').css('li')[0].css('td')
con.close if con
